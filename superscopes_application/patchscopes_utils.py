...
def inspect_amps(
  ...
  if module == "mlp":
    ...
    def store_mlp_hook(module, input, output):
      hs_cache_.append(output[0] * amplification)
...
def inspect_diffs(
  ...
  if module == "mlp_diff":
    ...
    def store_pre_mlp_hook(module, input):
      hs_cache_pre_.append(input[0])

    def store_post_mlp_hook(module, input, output):
      hs_cache_post_.append(output[0])

    for layer in mt.model.model.layers:
      store_hooks.append(layer.mlp.register_forward_pre_hook(store_pre_mlp_hook))
      store_hooks.append(layer.mlp.register_forward_hook(store_post_mlp_hook))

  elif module == "attn_diff":

    def store_post_attn_hook(module, input, output):
      hs_cache_.append(output[0].squeeze())

    for layer in mt.model.model.layers:
      store_hooks.append(layer.self_attn.register_forward_hook(store_post_attn_hook))
  ...
  if module == "mlp_diff":
    hs_cache_ = [
      hs_cache_post_[i] - hs_cache_pre_[i] for i in range(len(hs_cache_))
    ]
...
def inspect_amps_output_vec(
  ...
  return (output, hs_cache_[layer_source][position_source])
